...
cameraWidth = 320 #Width in pixels
horizontalAngle = 75 #Field of View IN DEGREES
degreesPerPixel = horizontalAngle/cameraWidth
actualObjectWidth = 100 #Actual object width IN CM
...
def processVision(cvSink, cvSrc):
    ...
    lower = np.array([37,25,167])
    upper = np.array([97,189,255])
    ...
    if(targetWidth != 0):
        targetAngle = targetWidth * degreesPerPixel
        targetAngleRad = targetAngle * (math.pi/180)

        dist = actualObjectWidth/(2*math.tan(targetAngleRad/2))

    sd = NetworkTables.getTable('SmartDashboard')
    sd.putNumber("targetWidth", targetWidth)
    sd.putNumber("targetAngle", targetAngle)
    sd.putNumber("distanceToObject", dist)
